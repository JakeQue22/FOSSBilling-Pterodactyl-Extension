...
class Box_Mod
{
    ...
    public function getService($sub = '')
    {
        ...
        if ($this->mod === 'servicepterodactyl') {     $class = 'FOSSBilling\\Mod\\' . ucfirst($this->mod) . '\\Service' . ucfirst($sub); } else {     $class = 'Box\\Mod\\' . ucfirst($this->mod) . '\\Service' . ucfirst($sub); }
        $service = new $class();
    ...
    if (in_array($this->mod, $coreModules)) {
        ...
        // Custom modules use FOSSBilling\Mod\
